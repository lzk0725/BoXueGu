...
public class UserInfoActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    private TextView tv_nickName,tv_sinnature,tv_user_name,tv_sex;
    private RelativeLayout rl_nickName,rl_sex,rl_signatrue,rl_title_bar;

    private static final int CHANGE_NICKNAME = 1;//修改昵称的自定义常量
    private static final int CHANGE_SIGNATRUE = 2;//修改个性签名的自定义常量
...
//    为控件设置值
    private void setValue(UserBean bean) {
      tv_nickName.setText(bean.nickName);
        ...
        tv_sinnature.setText(bean.signature);
    }
//控件点击监听事件
    ...
    public void onClick(View v) {
        switch (v.getId()){
            ...
            case R.id.rl_nickName://昵称点击事件
                String name = tv_nickName.getText().toString();//获取昵称空间上的数据
                Bundle dbName = new Bundle();
                dbName.putString("content",name);//传递界面上的昵称数据
                dbName.putString("title","昵称");
                dbName.putInt("flag",1);//flag传递1时表示是修改昵称
                enterActivityForResult(ChangeUserInfoActivity.class,
                        CHANGE_NICKNAME,dbName);//跳到个人资料界面
                break;
            case R.id.rl_sex://性别的点击事件
                String sex = tv_sex.getText().toString();//获取性别控件上的数据
                sexDialog(sex);
                break;
            case R.id.rl_signatrue://签名点击事件
                String signature = tv_sinnature.getText().toString();//获取签名控件上的数据
                Bundle dbSignatrue = new Bundle();
                dbSignatrue.putString("content",signature);//传递界面上的昵称数据
                dbSignatrue.putString("title","签名");
                dbSignatrue.putInt("flag",2);//flag传递1时表示是修改昵称
                enterActivityForResult(ChangeUserInfoActivity.class,
                        CHANGE_SIGNATRUE,dbSignatrue);//跳到个人资料界面
                break;
            default:
                break;
...
//设置性别弹出框
    private void sexDialog(String sex) {
        ...
        builder.setSingleChoiceItems(items,sexFlag, new DialogInterface.OnClickListener() {
            @Override
            public void onClick(DialogInterface dialog, int which) {//第二个参数时默认选中的哪个项
                dialog.dismiss();
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        switch (requestCode) {
            case CHANGE_NICKNAME:
                ...
                break;
                case CHANGE_SIGNATRUE://个人资料修改界面回传过来的签名数据
                    if (data != null) {
                        new_info = data.getStringExtra("signature");
                        if (TextUtils.isEmpty(new_info) || new_info == null) {
                            return;
                        }
                        tv_sinnature.setText(new_info);
                        //更新数据库中的签名字段
...
//获取回传数据时需要使用的跳转方法，
//    第一个参数to表示需要跳转的界面，第二个参数requesCode表示一个请求码，第三个参数b表示跳转时传递的数据
